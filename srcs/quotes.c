...
void del_after_quote(t_sh * sh)
{
    ...
    i = 0;
    while (sh->line->mode_str[i])
        i++;
    i--;
    while (sh->line->mode_str[i] && (sh->line->mode_str[i] != '\''))
        i--;
    tmp = ft_strsub(sh->line->mode_str, 0 , (i));
    ft_strdel(&sh->line->mode_str);
    sh->line->mode_str = tmp;
}
...
void simple_quote_work(t_sh *sh, int ret_stop[2])
{
    if (sh->buf == '\n')
        reset_line_mode(sh);
    if (ft_strchr(sh->line->mode_str, '\''))
    {
            ret_stop[1] = 1;
            set_term_back();
            del_after_quote(sh);
            sh->line->quote_complete = 1;
            ft_putstr(sh->line->mode_str);
    }
}
...
void del_quote(t_sh *sh)
{
    ...
    new[j] = '\n';
    new[j + 1] = '\0';
    sh->line->mode_str = ft_strdup(new);
}
    ...
    while (!ret_stop[1])
    {
        ...
        while ((ret_stop[0] = read(0, &sh->buf, sizeof(int))))
        {
            ...
            if (ft_isprint(sh->buf) || sh->buf == 10)
                sh->line->mode_str = ft_freejoinstr(sh->line->mode_str, ascii_to_char(sh->buf));
            if (sh->buf == 3)
            {
